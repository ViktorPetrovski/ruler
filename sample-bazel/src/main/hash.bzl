def _impl(ctx):
    ...
    ctx.actions.run_shell(
        ...
        inputs = [ctx.file.src],
        command = "echo md5sum {} > {}".format(ctx.file.src.path, ctx.outputs.md5.path),
    )

    ctx.actions.run_shell(
        outputs = [ctx.outputs.sha1],
        inputs = [ctx.file.src],
        command = "echo sha1sum {} > {}".format(ctx.file.src.path, ctx.outputs.sha1.path),
    )

    ctx.actions.run_shell(
        outputs = [ctx.outputs.sha256],
        inputs = [ctx.file.src],
        command = "echo sha256sum {} > {}".format(ctx.file.src.path, ctx.outputs.sha256.path),
    )
